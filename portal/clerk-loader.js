...
(function () {
  ...
  async function initialise(key) {
    ...
    let timer;
    try { await Promise.race([window.Clerk.load({ ui: { ClerkUI: window.__internal_ClerkUICtor }, appearance, localization: { unstable__errors: { external_account_not_found: "No account exists for this sign-in. Choose Create account to register first.", sign_up_if_missing_transfer: "No account exists for these details. Choose Create account to register first.", form_identifier_not_found: "No account was found for these details. Create an account first, then sign in.", form_identifier_exists__email_address: "An account with this email already exists. Sign in instead, or use Continue with Google if that is how you joined." } } }), new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('Secure sign-in took too long to start. Please try again.')), 15000); })]); }
