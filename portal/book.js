(async()=>{'use strict';if(window.SeanaPortalRedirecting)return;
 ...
 const appLink=document.getElementById('open-app');appLink.href='saunafinder://detail/'+encodeURIComponent(id)+'?'+new URLSearchParams({date,slotId,...(sessionTypeId?{sessionTypeId}:{})});appLink.hidden=false;
 ...
 const script=document.createElement('script');script.src='/portal/session-checkout.js?v=session-share-1';document.head.append(script);
