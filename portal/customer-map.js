...
window.createCustomerMap=({container,saunas})=>{
 ...
 const map=L.map(canvas,{preferCanvas:true,scrollWheelZoom:false,zoomAnimation:false,fadeAnimation:false,markerZoomAnimation:false}).setView([53.4,-8],6);
 ...
 if(bounds.length)map.fitBounds(bounds,{padding:[30,30],maxZoom:12,animate:false});
 }
